...
{ 
    ...
    class BlockBehaviorOmniRotatable : BlockBehavior {
		...
		private static Regex hrRe = new Regex(@".*-(?<dir>:north|south|east|west)(?:-.*)?");
		private static Regex vrRe = new Regex(@".*-(?<dir>:up|down)(?:-.*)?");
		private static Dictionary<string, string> flipTab = new Dictionary<string, string>(){
			{ "north", "south" },
			{ "south", "north" },
			{ "east", "west" },
			{ "west", "east" },
		...
		public override AssetLocation GetRotatedBlockCode(int angle, ref EnumHandling handling) {
			...
			if (m.Success) {
				// Frankly, I don't fully understand this. I ripped it out of the stairs code :P
				// Of course, I haven't really tried either...
				// TODO: Make sure I'm not missing an obvious shortcut or somesuch.
				BlockFacing newFacing = BlockFacing.HORIZONTALS_ANGLEORDER[((360 - angle) / 90 + BlockFacing.FromCode(m.Result("${dir}")).HorizontalAngleIndex) % 4];
