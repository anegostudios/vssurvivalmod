    ...
    public class BlockMeal : BlockContainer
    {
        ...
        public override void OnHeldInteractStop(float secondsUsed, ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel)
        {
            ...
            if (byEntity.World.Side == EnumAppSide.Server && multiProps != null && secondsUsed >= 0.95f)
            {
                ...
                int servingsLeft = GetQuantityServings(byEntity.World, eatenFromBowlStack);
                if (servingsLeft <= 1)
                {
                ...
                {
                    (eatenFromBowlStack.Collectible as BlockMeal).SetQuantityServings(byEntity.World, eatenFromBowlStack, servingsLeft - 1);
                ...
                EntityBehaviorHunger ebh = byEntity.GetBehavior<EntityBehaviorHunger>();
                ...
                {
                    mealSatpoints += nutriProps.Saturation;
                }

                float satMul = Math.Min(1, satiablePoints / mealSatpoints);
                satMul = (float)Math.Ceiling(satMul * 1000) / 1000;
        ...
        public void SetContents(string recipeCode, ItemStack containerStack, ItemStack[] stacks, int quantityServings = 1)
        {
            base.SetContents(containerStack, stacks);

            containerStack.Attributes.SetString("recipeCode", recipeCode);
            containerStack.Attributes.SetInt("quantityServings", quantityServings);
        }

        internal int GetQuantityServings(IWorldAccessor world, ItemStack byItemStack)
        {
            return byItemStack.Attributes.GetInt("quantityServings");
        }

        internal void SetQuantityServings(IWorldAccessor world, ItemStack byItemStack, int value)
        {
            byItemStack.Attributes.SetInt("quantityServings", value);
        }
