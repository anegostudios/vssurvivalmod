    ...
    public class BEBehaviorMPAxle : BEBehaviorMPBase
    {
        ...
        public static bool IsAttachedToBlock(IBlockAccessor blockaccessor, Block block, BlockPos Position)
        {
            ...
            if (orientations == "ns" || orientations == "we")
            {
                // Up or down
                if (
                    blockaccessor.GetBlockBelow(Position).SideSolid[BlockFacing.UP.Index] ||
                    blockaccessor.GetBlockAbove(Position).SideSolid[BlockFacing.DOWN.Index]
                ) return true;
            ...
            else
            {
                for (int i = 0; i < 4; i++)
                {
                    ...
                    Block blockNeib = blockaccessor.GetBlock(Position.X + face.Normali.X, Position.InternalY, Position.Z + face.Normali.Z);
        ...
        private bool RequiresStand(IWorldAccessor world, BlockPos pos, Vec3i vector)
        {
            try
            {
                BlockMPBase block = world.BlockAccessor.GetBlock(pos.X + vector.X, pos.InternalY + vector.Y, pos.Z + vector.Z) as BlockMPBase;
        ...
        private MeshData rotStand(MeshData mesh)
        {
            if (orientations == "ns" || orientations == "we")
            {
                ...
                if (!Api.World.BlockAccessor.GetBlockBelow(Position).SideSolid[BlockFacing.UP.Index])
                {
                    if (Api.World.BlockAccessor.GetBlockAbove(Position).SideSolid[BlockFacing.DOWN.Index])
                    {
                    ...
                    else if (orientations == "ns")
                    {
                        ...
                        if (Api.World.BlockAccessor.GetBlock(Position.X + face.Normali.X, Position.InternalY, Position.Z + face.Normali.Z).SideSolid[face.Opposite.Index])
                        {
