    ...
    public class BEBehaviorMicroblockSnowCover : BlockEntityBehavior, IRotatable, IMicroblockBehavior
    {
        ...
        private void GenSnowMesh()
        {
            ...
            if (SnowCuboids.Count > 0 && SnowLevel > 0)
            {
                ...
                {
                    SnowMesh.AddMeshData(BlockEntityMicroBlock.CreateMesh(Api as ICoreClientAPI, GroundSnowCuboids, new int[] { snowLayerBlockId }, beMicroBlock.OriginalVoxelCuboids, Pos));
                }
            }
            else
            {
