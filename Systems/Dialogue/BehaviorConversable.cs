...
namespace Vintagestory.GameContent
{
	...
	public class EntityBehaviorConversable : EntityBehavior
	{
		...
		public ConcurrentDictionary<string, DialogueController> ControllerByPlayer = [];
		public ConcurrentDictionary<string, DialogueConfig> _dialogueConfigs = [];
        ...
        public DialogueController? GetOrCreateController(EntityPlayer? player)
        {
            ...
            var dialogueConfig = GetDialogueConfig(dialogueLoc, player);
            ...
            foreach (var cmp in dialogueConfig.components)
            {
        ...
        DialogueConfig? GetDialogueConfig(AssetLocation baseLoc, EntityPlayer forPlayer)
        {
            var charClass = forPlayer.WatchedAttributes.GetString("characterClass");
            var ownPersonality = entity.WatchedAttributes.GetString("personality");

            var asset = world.AssetManager.TryGet(baseLoc.Clone().WithPathAppendixOnce($"-{ownPersonality}-{charClass}.json")) ??
                world.AssetManager.TryGet(baseLoc.Clone().WithPathAppendixOnce($"-{ownPersonality}.json")) ??
                world.AssetManager.TryGet(baseLoc.WithPathAppendixOnce(".json"));
            if (asset == null)
            {
                world.Logger.Error($"Entitybehavior conversable for entity {entity.Code}, dialogue asset {baseLoc} not found. Won't load dialogue.");
                return null;
            }

            try
            {
                return _dialogueConfigs.GetOrAdd(asset.Location, _ => {
		...
		public override void OnReceivedClientPacket(IServerPlayer player, int packetid, byte[] data, ref EnumHandling handled)
		{
			...
			if (packetid == SelectAnswerPacketId)
			{
                ...
                //TODO : controller might be null here
