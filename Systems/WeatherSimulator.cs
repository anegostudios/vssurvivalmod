...
namespace Vintagestory.GameContent
{
    public class WeatherSnapshot
    {
        ...
        public Dictionary<long, float[]> Weather = new Dictionary<long, float[]>();
    }
    ...
    public class WeatherSimulator
    {
        ...
        NormalizedPerlinNoise[] NoiseGens;
        ...
        public WeatherSimulator(int worldSeed, IWorldAccessor world)
        {
            ...
            {
                NoiseGens[i] = NormalizedPerlinNoise.FromDefaultOctaves(3, 0.1, 0.8, worldSeed + i + 1231212);
            }
        }
        ...
        public void OnUpdate(float dt)
        {
            ...
            for (int i = 0; i < players.Length; i++)
            {
                ...
                for (int j = 0; j < points.Length; i++)
                {
                    ...
                    if (!snap2.Weather.ContainsKey(chunkIndex2d))
                    {
                        snap2.Weather[chunkIndex2d] = CalcWeatherAt(points[i].X, points[i].Y, snap2.hoursTotal);
                    }
                }
            }
        }
        ...
        public bool TryGetWeather(double x, double z, ref float[] weather)
        {
            ...
            success = success && snap1.Weather.TryGetValue(topleft, out wtl1);
            success = success && snap1.Weather.TryGetValue(topright, out wtr1);
            success = success && snap1.Weather.TryGetValue(bottomright, out wbr1);
            success = success && snap1.Weather.TryGetValue(bottomleft, out wbl1);

            success = success && snap2.Weather.TryGetValue(topleft, out wtl2);
            success = success && snap2.Weather.TryGetValue(topright, out wtr2);
            success = success && snap2.Weather.TryGetValue(bottomright, out wbr2);
            success = success && snap2.Weather.TryGetValue(bottomleft, out wbl2);
