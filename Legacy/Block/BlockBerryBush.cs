...
namespace Vintagestory.GameContent
{
    public class BlockBerryBush : BlockPlant
    {
            ...
            interactions = ObjectCacheUtil.GetOrCreate(api, "berryBushInteractions", () =>
            {
                return new WorldInteraction[]
                {
                    new WorldInteraction()
                    {
                        ...
                        Itemstacks = ObjectCacheUtil.GetToolStacks(api, EnumTool.Shears),
                        GetMatchingStacks = (wi, bs, es) =>
                        {
                            if (GetBlockEntity<BlockEntityBerryBush>(bs.Position)?.Pruned == false) return wi.Itemstacks;
                            else return null;
                        }
                    }
                };
        ...
        public override bool OnBlockInteractStart(IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSel)
        {
            ...
            if ((!bebush?.Pruned ?? false) && byPlayer?.InventoryManager?.ActiveTool == EnumTool.Shears)
            {
                bebush.Prune();
                return true;
            }
