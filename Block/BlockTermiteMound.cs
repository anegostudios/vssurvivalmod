...
namespace Vintagestory.GameContent
{
    public class BlockTermiteMound : BlockRequireSolidGround
    {
        ...
        public override bool TryPlaceBlockForWorldGen(IBlockAccessor blockAccessor, BlockPos pos, BlockFacing onBlockFace, IRandom worldGenRand, BlockPatchAttributes attributes = null)
        {
            ...
            if (islarge)
            {
                if (!blockAccessor.GetBlock(pos.X - 1, pos.InternalY - 1, pos.Z - 1).SideSolid[BlockFacing.UP.Index]) return false;
                if (!blockAccessor.GetBlock(pos.X + 0, pos.InternalY - 1, pos.Z - 1).SideSolid[BlockFacing.UP.Index]) return false;
                if (!blockAccessor.GetBlock(pos.X + 1, pos.InternalY - 1, pos.Z - 1).SideSolid[BlockFacing.UP.Index]) return false;
                if (!blockAccessor.GetBlock(pos.X + 1, pos.InternalY - 1, pos.Z + 0).SideSolid[BlockFacing.UP.Index]) return false;
                if (!blockAccessor.GetBlock(pos.X + 1, pos.InternalY - 1, pos.Z + 1).SideSolid[BlockFacing.UP.Index]) return false;
                if (!blockAccessor.GetBlock(pos.X + 0, pos.InternalY - 1, pos.Z + 1).SideSolid[BlockFacing.UP.Index]) return false;
                if (!blockAccessor.GetBlock(pos.X - 1, pos.InternalY - 1, pos.Z + 1).SideSolid[BlockFacing.UP.Index]) return false;
                if (!blockAccessor.GetBlock(pos.X - 1, pos.InternalY - 1, pos.Z - 0).SideSolid[BlockFacing.UP.Index]) return false;
