        ...
        public override void OnLoaded(ICoreAPI api)
        {
            ...
            interactions = ObjectCacheUtil.GetOrCreate(api, "forgeBlockInteractions", () =>
            {
                ...
                foreach (CollectibleObject obj in api.World.Collectibles)
                {
                    ...
                    {
                        if (obj.GetHandBookStacks(capi) is List<ItemStack> stacks) heatableStacks.AddRange(stacks);
                    }
                    else if (obj.CombustibleProps?.BurnTemperature > 1000)
                    {
                        if (obj.GetHandBookStacks(capi) is List<ItemStack> stacks) coalStacklist.AddRange(stacks);
                    }
                }
